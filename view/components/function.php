...
function loginFromSocialCallBack($socialUser)
 {
    include '../CNM/Model/components/Connect.php';
    $result = mysqli_query($conn, "Select `TenDangNhap`,`Email`, from `taikhoan` WHERE `Email` ='" . $socialUser['Email'] . "'");
    if ($result->num_rows == 0) {
        $result = mysqli_query($conn, "INSERT INTO `taikhoan` (`TenDangNhap`,`Email` ) VALUES ('" . $socialUser['name'] . "', '" . $socialUser['Email'] . "', 1, " . time() . ", '" . time() . "');");
        if (!$result) {
