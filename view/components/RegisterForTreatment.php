...
<?php
 include "../Controller/components/cHeathDeclaration.php";
 $hoso=ControlSelecthsbn(1);
//  echo "<pre>";
//         print_r($hoso);
//      echo "</pre>";
?>
...
<div></div>
    <form action="index.php?act=dangkydieutri" method="POST">
        ...
        <div class="tokhai row">
            <label for="txtcoso" class="col-md-4 text-center">Mã cơ sở y tế</label>
            <select name="txtcoso" class="form-control col-md-7" id="mySelect" onchange="myFunction()">
            <option value="">---</option>
            <?php
                foreach($RegisterForTreatment as $tencsyt)
                {
                echo "<option value='".$tencsyt["TenCSYT"]."'>".$tencsyt["MaCSYT"]."</option>";
                }
            ?>
            </select>
        </div>
        <div class="tokhai row">
            <label for="txttt" class="col-md-4 text-center">Tên cơ sở y tế:</label>
        ...
        <div class="tokhai row">
            ...
            <input type="text" class="form-control col-md-7" name="hosobn" value="<?= $hoso['MaHoSo']?>">
        </div>
        <div class="tokhai row">
            <label for="txttinhtrang" class="col-md-4 text-center">Tình trạng bệnh:</label>
            <input type="text" class="form-control col-md-7" name="txttinhtrang" value="<?= $hoso['TinhTrangSucKhoe']?>">
